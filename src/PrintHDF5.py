...
def print_time(ds,ind):
    ...
    if item_last_name == 'time' :
       ...
       #tgmt = time.gmtime(tarr[0])    # converts sec to tuple struct_time in UTC
        print('Special stuff for "time" :',tarr[0],'sec,',  tarr[1],'nsec, ', end=' ') #, time.ctime(int(tarr[0]))
...
def iterate_over_data_structure(ds,offset0=''):
    ...
    print(offset, 'ds.shape =', ds.shape, '  len(ds.shape) =', len(ds.shape), '  shape dimension(s) =', end=' ')
    if len(ds.shape) == 0 :
